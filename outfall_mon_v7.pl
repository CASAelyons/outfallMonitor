...
sub file_monitor {
	...
	foreach $file (@new_files) {
		...
		if ($nDelts > 12) {
			...
			my $sth = $dbh->prepare("UPDATE outfalls SET BaseFlow='${baseflow}' WHERE Id = '${outfall_id}';");
			my $rv = $sth->execute
			    or warn $sth->errstr;
			$sth->finish;
		    ...
		    $sth = $dbh->prepare("UPDATE outfalls SET BaseFlow='${baseflow}' WHERE Id = '${outfall_id}';");
		    $rv = $sth->execute
			or warn $sth->errstr;
		    $sth->finish;

		    $sth = $dbh->prepare("UPDATE events SET BaseFlow='${baseflow}' WHERE EventId = '${eventid}';");
		    $rv = $sth->execute
			or warn $sth->errstr;
		    $sth->finish;
		    ...
		    $flow_endtime = $timearr[-1];
		    my $endtstamptz = timestringToTimestampTZ($flow_endtime);
