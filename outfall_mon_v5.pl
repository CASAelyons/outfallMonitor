...
sub file_monitor {
	...
	foreach $file (@new_files) {
		...
		while (my $line = <$data>) {
			...
			my $latest_delta = abs($fields[$field_to_read]-$latest_flow);
			$latest_flow = $fields[$field_to_read];
			if ($initial_delta == 0) {
			    if ($latest_delta > .1) {
				...
				my $lastEpoch = timestringToEpoch($timestamps[-2]);
				my $nowEpoch = timestringToEpoch($timestamps[-1]);
				
				print "last: " . $timestamps[-2] . " now: " . $timestamps[-1] . "\n";
				...
				if ($waitingPeriodGaugeTotal > .1) {
					...
					$inittstamptz = timestringToTimestampTZ($timefield);
				...
				my $endEpoch = timestringToEpoch($timestamps[-1]);
				...
				for (my $t = 0; $t < $nDelts; $t++) {
				    if ($delta_ts[$t] > .1) {					
					$num_fluctuations = $num_fluctuations + 1;
				    }
				}
				if ($num_fluctuations > 2) {
				    shift @delta_ts;
				    shift @timestamps;
				    next;
				}
